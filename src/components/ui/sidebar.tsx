...
function Sidebar({
  ...
  // Keep desktop shell until mounted so SSR and first client paint match.
  if (mounted && isMobile) {
    return (
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      {...props}
    >
      <PanelLeftIcon className="rtl:rotate-180" suppressHydrationWarning />
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  // Defer until mount so SSR/first paint never diverge (Cursor tooling + RTL class variants).
  if (!mounted) {
    return null;
  }
...
function SidebarMenuButton({
  ...
  const mounted = useMounted();
  // Tooltips only after mount — avoids TooltipTrigger wrapper mismatch on hydrate.
  const showTooltip =
    mounted && Boolean(tooltip) && state === "collapsed" && !isMobile;
